...
The iRail API is a XML/JSON HTTP API where each request on \url{http://api.irail.be} returns a response that can be used directly in an end-user app.
It contains four features:

\begin{itemize}
\item Route Planning (\url{http://api.irail.be/connections/{?from,to,date,time}}).
\item Next Departures (\url{http://api.irail.be/liveboard/{?station,date,time}}).
\item Train Status (\url{http://api.irail.be/vehicle/{?id,date}}).
\item List All Stations (\url{http://api.irail.be/stations/}).
\end{itemize}

\todo{what are the intended functions of each feature? We should mention that the route planning feature is perhaps the most popular among lay users? Thus is also has good potential to be used as a proxy for analysis...}

We are particularly intersted in studying the potential application of route planning queries for flow analysis thus  only requests to (\url{http://api.irail.be/connections/{?from,to,date,time}}) were extracted for further investigation. The resulting dataset described in this paper, can be downloaded at \todo{\url{http://datawijs.be/apilog.tar.gz}}.

This dataset is compiled from the \emph{Apache} \emph{access.log} files generated between 2012 and 2015. Each data entry contains a: 
\begin{itemize}
  \item Timestamp,
  \item HTTP Status Code,
  \item Request Path (e.g., \url{/connections/?from=Ghent\&to=Antwerp})
  \item User-Agent (cfr. RFC2616\footnote{The User-Agent request-header field contains information about the user agent originating the request. This is for statistical purposes, the tracing of protocol violations, and automated recognition of user agents for the sake of tailoring responses to avoid particular user agent limitations -- \url{http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html}})
\end{itemize}
...
Flow analysis involves the comparison of aggregated movement between distinct locations within a specific time frame. Data visualisation is typically employed to facilate in this process thus the input data must be transformed into a format that expresses the quantity and directional flow of human movment. In this section we describe the data transformation procedure and data visualisation technique  employed in our case study.

\subsection{Processing}

We processed the the data so that for both every origin and destination we have a unique identifier and the geographic coordinates. This is facilitaed with the open-source reconciliator \todo{one liner that describes how this works} situated at \url{https://github.com/irail/stations}.
