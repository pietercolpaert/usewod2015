...
Yet, getting statistics of travel flows has proven to be troublesome.
The iRail project hosts a high expressive route planning API for the Belgian railway company.
% Task
We analyse the query logs of this API, to get an indication of travel flows between cities in Belgium.
We were able to study $\sim$100k to 500k requests for each month between October 2012 and November 2015, which amount to \todo{between $0.3\%$ and $2\%$} of the amount of monthly passengers.
% Object
Using data visualizations, we illustrate the commuting patterns in Belgium and show that Brussels, the captial, acts as a central hub. 
% Findings
The visualizations correspond to reality, as confirmed by experts. 
% Conclusion
More research is needed to understand how representative for travel flows query logs may be.
% Perspectives
...
The iRail API is a plain old XML/JSON HTTP API: one request documented for humans at $api.iRail.be$, will result in an answer that can be used directly in an end-user app.
It contains four features: a route planning query, a query for the next departures in a certain station, a query for the status of a certain train and a query for a list of all stations.
In 2013, we started to use a MySQL table to store each log entry to the iRail API in a table.
The columns of this table are:
\begin{itemize}
  \item User-Agent as defined by RFC2616\footnote{\url{http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html}}: \emph{the User-Agent request-header field contains information about the user agent originating the request. This is for statistical purposes, the tracing of protocol violations, and automated recognition of user agents for the sake of tailoring responses to avoid particular user agent limitations};
  \item the timestamp,
  \item whether or not the request succeeded,
  \item the departure station and
  \item the desired station of arrival.
\end{itemize}

\begin{figure}
\centering
\includegraphics[width=193,align=center]{querylogs}
...
\begin{figure}
\centering
\includegraphics[width=8.3cm,align=center]{avg-all}
...
%\begin{figure}
%\centering
%\includegraphics[width=8.3cm,align=center]{dec21}
...
\begin{figure}
\centering
\includegraphics[width=8.3cm,align=center]{dec24}
...
If we want to log every HTTP GET request, we expect every HTTP GET to reach the server and we would have to deliberately disable caches to be able to log every request.
The chance that two requests are exactly the same for a high expressive server is low, as there are more URLs which can be requested. 
We have shown in previous sections that these logs are interesting: each request contains an entire query which can be used as an indication of travel ``demand''.

A low expressive server that still publishes the data in a queryable way as illustrated in \cref{fig:lc}, has less interesting results. 
E.g., when it only allows to set a departure station and a departure time, then the server cannot know the arrival station, which makes it harder to get an indication of the travel flow.
Furthermore, on the Linked Data Fragments axis, less expressive servers --which have less URLs to be requested-- can exploit the caching mechanism~\cite{ldf}.
The drawback of having higher cacheability, is that intermediate proxies may have cached the HTTP response somewhere in between and there will not be a log entry.

When the expressiveness of the servers is lower in order to maximize the reuse of the data, such as in the case of Linked~Connections or a GTFS~datadump, we suggest to, instead of logging each GET-request, to have an analytics server that gathers all the logs.
...
% Open Anonymized Route Planning Query Logs
