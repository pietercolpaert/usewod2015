...
Assuming that each request reciprocates with an intention to travel by rail, our data captures a respective $0.56\%$, 0.88\%, 0.77\% and 1.66\% of the actual journeys that have occurred. This suggests that \todo{Pieter: what does this mean? The numbers appear rather small though}.
...
\cref{fig:average} depicts an break down of the queries that occur on an hourly basis for each day of the week.
...
Similarly, the frequency of queries during the morning peak period on Fridays appear to be substantially lower than those that occur between Monday and Thursday.
The discrepancy maybe explained by a decrease in the number students who travel home from college over the weekends. \todo{Can we give a link to more reading material about this behaviour? This might be weird for US people? --> not sure if this is really case. We need to explain this in a clear way}.
The absence of a clear peak on Saturdays can be attributed to lack of journey to work queries while the evening peak on Sundays may emerge from students returning to college accommodations.

Finally, the distribution of queries on public holidays is observed to deviate from that of an average day. 
Figure 3 provides a breakdown of queries per hour on Christmas eve, 24th December 2015, in comparison to an average Thursday of the same year. 
...
Visualization is crucial for making the analysis tangible, so that results can be communicated and debated \cite{robinson2008collaborative}. 
...
Movement data has received substantial attention from the visualisation community and a range of techniques has been developed to support analysis \cite{andrienko2012visual}. 
...
Brussels serves as the principal centre of rail activity in general, yet its function as a centre is more distinctive for the walloon region than Flanders.
...
%As a new version of the logs are put at \url{http://api.irail.be/logs/} only each second, we have added cache-headers to the document with max-age equal to 1 second.
%\todo{Also in USEWOD2014 there was a paper on LDF and how we could know which SPARQL queries needed to be solved}

\section{Future Work}

Query logs are thus an indication of travel intentions. 
Yet we can identify some hiatuses in query logs as travel intentions:
\begin{enumerate}
  \item Caching may mask peak hours, as \ldots
  \item Multiple requests may be needed to cover a travel intention
  \item \ldots
\end{enumerate}

In order to create a better representation of travel intentions, we would recommend apps to do a POST-request instead of a 
