...
% Perspectives
Nevertheless, we should be cautious about what we measure as GET requests on the Web are cacheable.
In order to have a query log for studying travel demand that works for all publishing strategies, we suggest future work to send anonymized POST requests from the end-user applications to an analytics server.
...
We are particularly intersted in studying the potential application of route planning queries for flow analysis.
The other queries are commonly used by digital signage providers or status boards, which poll the data on a regular basis.
