...
Visualizations are crucial to understanding the patterns in data representing transit flows.
...
\todo{PC: Alvin, can we have a citation for this paragraph? Maybe you can cite one your own papers here?}
...
%\todo{Maybe a sub header here?} - PC: I wouldn't do a subheader...
...
We have excluded such queries since we are specifically interested in studying human travel behavior.
...
We employ an open-source reconciliator (\url{https://github.com/irail/stations}) to reduce spelling variations and remove data records with missing information. 
Additionally, station names are automatically linked to their correspodinging geographical coordinates through this process.

\begin{figure*}
  \centering
  \includegraphics[width=14cm]{method-c.pdf}\\
  \caption{Spatial aggregation based on provincial administrative boundaries. Each station is color coded to indicate the province it will be aggregated to. Stations in major cities as well as stations outside of Belgium are aggregated to seperate distinct groups.}
  \label{fig:method-c}
\end{figure*}
...
\begin{figure}[h]
  ...
  \begin{subfigure}{.2375\textwidth}
    ...
    \subcaption{Lines and dots\todo{Alvin}} % 4 words max
    \label{fig:method-a}
  \end{subfigure}%
  \begin{subfigure}{.2375\textwidth}
    \includegraphics[width=4cm]{method-b.pdf}\\
    \subcaption{arrows and dots\todo{Alvin}} % 4words max
    \label{fig:method-b}
  \end{subfigure}%
  \vspace{5.0pt}% To create a bit of vertical space between the 2 figure captions
  \caption{With the arrow and dots and the lines and dots you can see that TODO \todo{Alvin: can you write a caption here?}}
\end{figure}

Next, the data is aggregated so that province level flow patterns are emphasized.
\cref{fig:method} provides a diagrammatic representation of the procedure.
First, both origin and destination stations for each data record are spatially aggregated based on provincial administrative boundaries (see \cref{fig:method-a}).
Train stations in major cities are excluded from aggregation so that the volume of flow between provinces and major cities are comparable (see \cref{fig:method-b}).
If an origin or destination station is a located outside of Belgium, it is considered international travel and aggregated in a distinct group (see \cref{fig:method-c}).
...
%Isn't this sufficient ↑? \todo{Trein Tram Bus needs to be introduced in the introduction}
