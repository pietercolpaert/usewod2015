...
\begin{itemize}
\item Route Planning Feature (\url{http://api.irail.be/connections/{?from,to,date,time}}).
\item Next Departures Feature (\url{http://api.irail.be/liveboard/{?station,date,time}}).
\item Train Status Feature query (\url{http://api.irail.be/vehicle/{?id,date}}).
\item List All Stations Feature (\url{http://api.irail.be/stations/}).
\end{itemize}
...
We have gathered the \emph{Apache} \emph{access.log} files generated between 2012 and 2015 containing: 
\begin{itemize}
  \item Timestamp,
...
We processed the the data so that for both every origin and destination we have a unique identifier and the longitude and latitude.
We have done this using the open-source reconciliator at \url{https://github.com/irail/stations}.
